...
| INST | `_instance_id` used at 3 call sites via `getattr(self, "_instance_id", "")` but never defined - always empty string (certificates embed empty ID) | **FIXED** - `self._instance_id = os.environ.get("ORGANISM_INSTANCE_ID", "")` added to `__init__` |
