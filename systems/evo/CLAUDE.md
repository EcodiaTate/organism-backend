...
- **No hard niche cap** - `_MAX_NICHES` constant removed from `cognitive_niche.py`. Niche count is gated only by the metabolic starvation signal (the natural limit) not by an arbitrary number. `EvoConfig.max_niches` exists in config.py as an opt-in safety valve (default 0 = unlimited).
...
| `OIKOS_ECONOMIC_EPISODE` | Oikos | `_on_oikos_economic_episode()` - consumes action_type, success, roi_pct, protocol, causal_applicable_domains; creates COOCCURRENCE `PatternCandidate` in `_pending_candidates`; confidence = 0.20 (failure) or 0.40–0.80 scaled by roi_pct (success) |
| `METABOLIC_GATE_RESPONSE` | Oikos | `_on_metabolic_gate_response()` - denied gates only; creates TEMPORAL `PatternCandidate` with elements `[metabolic_gate_denied::{action_type}, starvation_level::{level}, economic_constraint]`; confidence=0.35 |
